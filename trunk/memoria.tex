...
garant\'ias de seguridad.
	
Debido al aspecto inter-dominio de BGP, incluso peque\~nas fallas dentro
de un AS pueden, en ocasiones, propagarse ampliamente al resto de
...
cre\'o Pretty Good BGP~\cite{PGBGP1,PGBGP2} (PGBGP) un sistema que no conf\'ia 
autom\'aticamente en las rutas, sino que trata de dar tiempo para saber si son 
reales antes de anunciarlas c\'omo ruta \'optima.
...
se puede saber cual es el rango de IP que se encuentra en Chile; para de esta 
forma hacer el estudio centrado espec\'ificamente en nuestro pa\'is.
...
Cooperative Association for Internet Data Analysis (CAIDA), es una asociaci\'on que provee herramientas y an\'alisis promoviendo el mantenimiento de una 
...
Asia-Pac\'ifico, pero en este estudio, no se incluy\'o Chile, por lo tanto es 
algo beneficioso poder ver nuestra realidad local.
...
~\cite{uorvp}, los archivos RIPE ~\cite{ripe} y el Monitor BGP del MIT
...
obtener informaci\'on o estad\'isticas de ellos. BGP-Inspect 
~\cite{bgp-inspect} es una herramienta que obtiene y preprocesa informaci\'on 
de estos archivos, proporcionando una interfaz de consulta m\'as simple desde
la cual se puede obtener informaci\'on agregada.
